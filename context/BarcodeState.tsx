...
// Reducer function
const barcodeReducer = (state: BarcodeState, action: BarcodeAction): BarcodeState => {
    switch (action.type) {
        case 'SET_SCANNING_RESULT':
            return { ...state, scannedData: action.payload, scannedProduct: undefined};
        case 'RESET_SCAN':
...
export const BarcodeProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    useEffect(() => {
       ...
       const loadUpcData =  async (type: BarcodeType, upc: string) => {
           ...
           if (type  == "upc_e" || type == "upc_a" || type == "ean8" || type == "ean13") {
